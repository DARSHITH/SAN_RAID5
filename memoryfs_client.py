...
class DiskBlocks():
  def __init__(self, args):
    ...
    if args.port0 or args.port1 or args.port2 or args.port3 or args.port4 or args.port5 or args.port6 or args.port7:
      if args.port0:
        PORT0 = args.port0
        s0 = 'http://' + SERVER_ADDRESS + ':' + str(PORT0)
        self.block_server += [xmlrpc.client.ServerProxy(s0, use_builtin_types=True)]
      if args.port1:
        PORT1 = args.port1
        s1 = 'http://' + SERVER_ADDRESS + ':' + str(PORT1)
        self.block_server += [xmlrpc.client.ServerProxy(s1, use_builtin_types=True)]

      if args.port2:
        PORT2 = args.port2
        s2 = 'http://' + SERVER_ADDRESS + ':' + str(PORT2)
        self.block_server += [xmlrpc.client.ServerProxy(s2, use_builtin_types=True)]

      if args.port3:
        PORT3 = args.port3
        s3 = 'http://' + SERVER_ADDRESS + ':' + str(PORT3)
        self.block_server += [xmlrpc.client.ServerProxy(s3, use_builtin_types=True)]

      if args.port4:
        PORT4 = args.port4
        s4 = 'http://' + SERVER_ADDRESS + ':' + str(PORT4)
        self.block_server += [xmlrpc.client.ServerProxy(s4, use_builtin_types=True)]

      if args.port5:
        PORT5 = args.port5
        s5 = 'http://' + SERVER_ADDRESS + ':' + str(PORT5)
        self.block_server += [xmlrpc.client.ServerProxy(s5, use_builtin_types=True)]

      if args.port6:
        PORT6 = args.port6
        s6 = 'http://' + SERVER_ADDRESS + ':' + str(PORT6)
        self.block_server += [xmlrpc.client.ServerProxy(s6, use_builtin_types=True)]

      if args.port7:
        PORT7 = args.port7
        s7 = 'http://' + SERVER_ADDRESS + ':' + str(PORT7)
        self.block_server += [xmlrpc.client.ServerProxy(s7, use_builtin_types=True)]

    else:
      print('Must specify port number')
      quit()
    if len(self.block_server)!=args.total_num_servers:
      print('Number of ports not matching with the ports declared')
      quit()


    #server_url = 'http://' + SERVER_ADDRESS + ':' + str(PORT0)
  ...
  def HandleFSConstants(self, args):
    ...
    if args.total_num_servers:
      global TOTAL_NUM_SERVERS
      TOTAL_NUM_SERVERS = args.total_num_servers
...
## Blocks are padded with zeroes up to BLOCK_SIZE
  ...
  # def Raid4map(self,block_number):
  #   if TOTAL_NUM_SERVERS == 1:
  #     r4map = [block_number,0]
  #     return r4map
  #   else:
  #     p_block_number = block_number//TOTAL_NUM_SERVERS-1
  #     blockserver_id = block_number%(TOTAL_NUM_SERVERS-1)
  #     r4map=[p_block_number,blockserver_id]
  #     return r4map

  def Repair(self,serverID):
    if serverID > TOTAL_NUM_SERVERS:
      return -1
    for i in range(0, (TOTAL_NUM_BLOCKS//(TOTAL_NUM_SERVERS-1))):
      repairedData = bytes(bytearray(BLOCK_SIZE))
      for j in range(0, TOTAL_NUM_SERVERS):
        if j == serverID:
          continue
        else:
          try:
            blockData = self.block_server[j].Get(i)
            if blockData == -1:
              print("Multiple faults occurred")
              quit()
            repairedData = bytes([a ^ b for a, b in zip(repairedData, bytes(blockData))])
          except ConnectionRefusedError:
            print("Multiple faults occurred")
            quit()
      self.block_server[serverID].Put(i, repairedData)

  def Raid5map(self,block_number):
    p_block_number = block_number//(TOTAL_NUM_SERVERS-1)
    Parityserver_id = (TOTAL_NUM_SERVERS-1) - (p_block_number % TOTAL_NUM_SERVERS)
    blockserver_id = block_number % (TOTAL_NUM_SERVERS-1)
    if blockserver_id >= Parityserver_id:
      blockserver_id += 1
    r5map = [p_block_number, blockserver_id, Parityserver_id]
    return r5map

  def correctBlock(self,block_number):
    raid5mapping = self.Raid5map(block_number)
    corrected = bytes(bytearray(BLOCK_SIZE))
    #corrected = bytearray(block_data.ljust(BLOCK_SIZE,b'\x00'))
    for i in range(0,TOTAL_NUM_SERVERS):
      if i == raid5mapping[1]:
        continue
      else:
        #print("correcting block no: "+str(block_number))
        try:
          blockData = self.block_server[i].Get(raid5mapping[0])
          if blockData == -1:
            print("More than one block is corrupted")
            quit()
          corrected = bytes([a ^ b for a, b in zip(corrected, bytes(blockData))])
        except ConnectionRefusedError:
          print("Multiple faults occurred")
          quit()
    return bytearray(corrected)
  ...
  def Put(self, block_number, block_data):
    ...
    if block_number in range(0,TOTAL_NUM_BLOCKS):
      ...
      raid5mapping = self.Raid5map(block_number)
      try:
        oldData = self.block_server[raid5mapping[1]].Get(raid5mapping[0])
        if oldData == -1:
          oldData = self.correctBlock(block_number)
        try:
          oldParity = self.block_server[raid5mapping[2]].Get(raid5mapping[0])
          if oldParity == -1:
            correctedParity = bytes(bytearray(BLOCK_SIZE))
            for i in range(0, TOTAL_NUM_SERVERS):
              if i == raid5mapping[2]:
                continue
              else:
                try:
                  ...
                  if blockData == -1:
                    print("Multiple faults occurred")
                    quit()
                  correctedParity = bytes([a ^ b for a, b in zip(correctedParity, bytes(blockData))])
                except:
                  print("Multiple faults occurred")
                  quit()
            oldParity = correctedParity
          newParity = bytes([a ^ b for a, b in zip(bytes(putdata), bytes(oldData))])
          parityData = bytes([a ^ b for a, b in zip(bytes(newParity), bytes(oldParity))])
          self.block_server[raid5mapping[2]].Put(raid5mapping[0], parityData)
          #print("Parity of "+str(raid5mapping[1])+" updated on "+str(raid5mapping[2]))
          self.block_server[raid5mapping[1]].Put(raid5mapping[0], putdata)
          return 0
        except ConnectionRefusedError:
          self.block_server[raid5mapping[1]].Put(raid5mapping[0], putdata)
          return 0
        ...
        try:
          ...
          print("More than one server is down")
          quit()
    else:
...
## Equivalent to the textbook's BLOCK_NUMBER_TO_BLOCK(b)
  ...
  def ServerGet(self, block_number):
    ...
    if block_number in range(0,TOTAL_NUM_BLOCKS):
      # logging.debug ('\n' + str((self.block[block_number]).hfex()))
      # commenting this out as the request now goes to the server
      # return self.block[block_number]
      # call Get() method on the server
      raid5mapping=self.Raid5map(block_number)
      try:
        data = self.block_server[raid5mapping[1]].Get(raid5mapping[0])
      except ConnectionRefusedError:
        data = self.correctBlock(block_number)
      # return as bytearray
      if data != -1:
        return bytearray(data)
      else:
        #print("corrupt block detected : "+str(block_number))
        data = self.correctBlock(block_number)
        return bytearray(data)
...
## RSM: read and set memory equivalent
  ...
  def RSM(self, block_number):
    ...
    if block_number in range(0,TOTAL_NUM_BLOCKS):
      raid5mapping=self.Raid5map(block_number)
      try:
        data = self.block_server[raid5mapping[1]].RSM(raid5mapping[0])
      except ConnectionRefusedError:
        data = self.correctBlock(block_number)
      return bytearray(data)
